...
const query = "SELECT id, entityid, firstname, lastname, email, issalesrep FROM employee WHERE lastname LIKE '%' ORDER BY lastname, firstname FETCH FIRST 50 ROWS ONLY";
console.log('Searching for all employees...\n');
executeSuiteQL(query).then(result => {
  if (result.statusCode === 200 && result.data.items) {
    console.log('Found', result.data.items.length, 'employees:\n');
    result.data.items.forEach((emp, index) => {
      const fullName = (emp.firstname || '') + ' ' + (emp.lastname || '');
      console.log((index + 1) + '. ' + fullName.trim());
      console.log('   ID: ' + emp.id + ', Entity: ' + emp.entityid);
      console.log('   Email: ' + (emp.email || 'N/A'));
      console.log('   isSalesRep: ' + (emp.issalesrep === 'T' ? 'true' : 'false'));
      console.log('');
    });
  } else {

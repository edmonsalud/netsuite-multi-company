...
// Search for entities that look like sales reps (employees)
const names = ['Eric', 'Garrett', 'Greg', 'Jeff', 'Kim', 'Matt', 'Mike', 'Sandy', 'Pearle', 'Dixon', 'Landers', 'McKendree', 'Lyons', 'Macleod', 'McCarn', 'Hopkins'];

console.log('Searching entity table for potential employees...\n');

const namePattern = names.map(n => "entityid LIKE '%" + n + "%'").join(' OR ');
const query = "SELECT id, entityid, type FROM entity WHERE (" + namePattern + ") FETCH FIRST 50 ROWS ONLY";
...
executeSuiteQL(query).then(result => {
  if (result.statusCode === 200 && result.data.items) {
    ...
    result.data.items.forEach((ent, index) => {
      console.log((index + 1) + '. ID: ' + ent.id + ', Entity ID: ' + ent.entityid + ', Type: ' + (ent.type || 'N/A'));
    });
  } else {

...
const query = "SELECT id, entityid, companyname, email FROM customer WHERE UPPER(companyname) LIKE '%GOODBYE STRANGER%'";
console.log('Searching for: GOODBYE STRANGER COLLAR FREE PRODUCTIONS\n');
executeSuiteQL(query).then(result => {
  if (result.statusCode === 200 && result.data.items) {
    console.log('Found', result.data.items.length, 'customer(s):\n');
    ...
    console.log('Error or no results:', JSON.stringify(result.data, null, 2));
  }

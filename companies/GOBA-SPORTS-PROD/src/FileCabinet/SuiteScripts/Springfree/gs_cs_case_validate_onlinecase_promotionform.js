 ...
 * 2.50		19-08-22	R.Basile		Created following function for NZ Promotion form (NZ)
 * 											- function promo_SaveRecord()
 * 											- function promo_ValidateFields(enable_mssg)
 */
...
function promo_SaveRecord(){	
	return promo_ValidateFields(false);	
}
...
function validate_WarrantyFields(subsidiary){
	...
	if(isNULL(nlapiGetFieldValue('custevent_sn_frame'))){
		if(isNULL(errorMssg)){
			errorMssg = '  Frame Section';
		}
		else{
			errorMssg += '\n' + '  Frame Section';
		}
	}	
	if(isNULL(nlapiGetFieldValue('custevent_sn_enclosure'))){
		if(isNULL(errorMssg)){
			errorMssg = '  Enclosure';
		}
		else{
	...
	if(isNULL(nlapiGetFieldValue('custevent_sn_mat'))){
		if(isNULL(errorMssg)){
			errorMssg = '  Mat';
		}
		else{

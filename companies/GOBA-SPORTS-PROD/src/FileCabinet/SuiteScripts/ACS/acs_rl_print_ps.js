...
define(['N/record', 'N/render', 'N/search'], function (record, render, search) { return { get: function get(context) {
    log.audit('🏴 get(context)', context);

    if (!context.id) throw "<h1>No Record ID</h1>";
    var createdFrom = context.createdFrom;
    ...
    /*
        RESPONSE section
    */
    log.debug('Here comes pdf...');

    if (context.mode == 'string') {
        log.debug('string', JSON.stringify({ xmlString: objTemplateRenderer.renderAsString() }));
        return JSON.stringify({ xmlString: objTemplateRenderer.renderAsString() });

    } else {
        log.debug('binary');
        const fileObj = objTemplateRenderer.renderAsPdf();
        fileObj.name = `ItemShip_${context.id}.pdf`;
        fileObj.folder = context.folder;
        fileObj.isOnline = true;
        const fileId = fileObj.save();
        log.debug('link', {fileId});
